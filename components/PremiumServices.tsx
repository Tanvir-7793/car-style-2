            ...
            <div className="max-w-7xl mx-auto px-8">
                <div className="text-center max-w-3xl mx-auto mb-16 flex flex-col items-center">
                    <ScrollFloat
                        ...
                        stagger={0.03}
                    >
                        <span className="text-primary font-bold uppercase tracking-widest text-sm block mb-4">{t("services.badge")}</span>
                    ...
                    {displayedServices.map((service, i) => (
                        <div key={i} className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all border border-black/5 group">
                            ...
                            <div className="p-8">
                                ...
                                <ul className="space-y-2 mb-5 border-t border-black/5 pt-3">
                                    {service.features.map((feature, j) => (
                                        <li key={j} className="flex items-center gap-2 text-xs font-semibold text-gray-500">
                                            <CheckCircle2 className="w-4 h-4 text-primary" /> {t(feature)}
                                        </li>
                                    ))}
                                </ul>
                                <Link
                                ...
                                >
                                    {t("cta.bookService")} <ChevronRight className="w-4 h-4" />
                                </Link>
                            </div>
                        </div>
                    ))}
